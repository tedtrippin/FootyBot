...
public class RemoveFilterController extends BaseController {

    private final Logger log = Logger.getLogger(RemoveFilterController.class);

    @RequestMapping(method = RequestMethod.GET)
    public ModelAndView getRemoveFilter(@CookieValue(value = ModelKeys.BOT) String botString, @RequestParam("index") String index,
        HttpServletResponse response) {

        log.debug("getRemoveFilter");

        try {
            int idx = Integer.parseInt(index);
            Bot bot = cookieToBot(botString);
            if (idx < bot.getFilters().size()) {
                bot.getFilters().remove(idx);
